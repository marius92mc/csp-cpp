...
def test_cpp_generator_channel_outside_select_case_write(tmpdir: local.LocalPath) -> None:
    def _get_input_cpp() -> str:
        return '#include <iostream>\n' \
               '\n' \
    ...
    _add_content(tmpdir, _get_input_cpp())
    select_data = SelectParser(_get_file_path(tmpdir)).parse()
    ...
    with open(tmpdir.join(_get_generated_file_name()), "r") as generated:
        assert generated.read() == _get_output_cpp()
        verified = True
    assert verified
